...
describe('Overlay Component with Default Config', () => {
    ...
    describe('Check the inputs', () => {
        ...
        describe(`should check "useBootstrapStyles"`, () => {
            const setCustomStyles = (): void => {
                component.config = {
            ...
            it(`should check the basics for "useBootstrapStyles"`, () => {
                const element: DebugElement = getOverlay();

                fixture.detectChanges();
                expect(component.overlay.useBootstrapStyles).toBe(false);
                expect(DOMHelper.getAttributeValue(element.nativeElement, 'use-bs')).toBe(null);
                expect('popover' in element.classes).toBe(false);

                component.config = { useBootstrapStyles: true };
                fixture.detectChanges();

                expect(component.overlay.useBootstrapStyles).toBe(true);
                expect(DOMHelper.getAttributeValue(element.nativeElement, 'use-bs')).toEqual('');
                expect('popover' in element.classes).toBe(true);

                component.config = { useBootstrapStyles: 'some-value' as unknown as boolean };
                fixture.detectChanges();

                // Even by setting 'some-value', it should respect the last value set, that was 'true'
                expect(component.overlay.useBootstrapStyles).toBe(true);

                component.config = { useBootstrapStyles: false };
                fixture.detectChanges();
                ...
                // Even by setting 'some-value', it should respect the last value set, that was 'false'
                expect(component.overlay.useBootstrapStyles).toBe(false);
            });

            it(`should check the custom styles are not saved when "useBootstrapStyles" is set to "true"`, () => {
                setCustomStyles();
                component.config.useBootstrapStyles = true;
                fixture.detectChanges();

                const directiveConfig = component.overlay['directiveConfig'];

                expect(directiveConfig.backgroundColor).toEqual(OVERLAY_DEFAULTS.backgroundColor);
                expect(directiveConfig.textColor).toEqual(OVERLAY_DEFAULTS.textColor);
                expect(directiveConfig.borderColor).toEqual(OVERLAY_DEFAULTS.borderColor);
                expect(directiveConfig.borderRadius).toEqual(OVERLAY_DEFAULTS.borderRadius);
                expect(directiveConfig.shadow).toEqual(OVERLAY_DEFAULTS.shadow);
                expect(directiveConfig.shadowColor).toEqual(OVERLAY_DEFAULTS.shadowColor);
            });

            it(`should check the custom styles are saved when "useBootstrapStyles" is set to "false"`, () => {
                setCustomStyles();
                component.config.useBootstrapStyles = false;
                fixture.detectChanges();
                ...
                expect(directiveConfig.backgroundColor).toEqual('red');
                expect(directiveConfig.textColor).toEqual('yellow');
                expect(directiveConfig.borderColor).toEqual('orange');
                expect(directiveConfig.borderRadius).toEqual(20);
                expect(directiveConfig.shadow).toEqual('none');
                expect(directiveConfig.shadowColor).toEqual('gray');
            });
        });
...
@Component({
    selector: 'a11y-test-overlay-custom-config-component',
    template: `
        <div class="scroll-generator-height"></div>
        <button type="button" #trigger (click)="overlay.toggle()">trigger</button>
        <a11y-overlay #overlay="a11yOverlay" [trigger]="trigger" [config]="config">
...
export class CustomOverlayCustomConfigTestComponent {
    @ViewChild(OverlayComponent) overlay: OverlayComponent;

    config: OverlayCustomConfig = {
        selector: '.custom-test-overlay',
        className: 'my-custom-overlay',
    };
}

@NgModule({
    declarations: [CustomOverlayCustomConfigTestComponent],
    imports: [
        A11yOverlayModule.customConfig({
            ...
            allowClose: false,
            arrowSize: 10,
            className: ['custom-test-overlay', 'custom-config-overlay'],
            offsetSize: 10,
            position: 'bottom-start',
            positionStrategy: 'absolute',
            safeSpace: { top: 100, left: 100, right: 100, bottom: 100 },
...
export class CustomOverlayCustomConfigTestModule {}

describe('Overlay Component with Custom Config', () => {
    let component: CustomOverlayCustomConfigTestComponent;
    let fixture: ComponentFixture<CustomOverlayCustomConfigTestComponent>;
    let DOMHelper: DOMHelperService;
    ...
    beforeEach(waitForAsync(() => {
        TestBed.configureTestingModule({
            declarations: [CustomOverlayCustomConfigTestComponent],
            imports: [CustomOverlayCustomConfigTestModule],
            providers: [DOMHelperService],
            ...(forceStylesCleanup ? { teardown: { destroyAfterEach: true } } : {}),
        }).compileComponents();

        DOMHelper = TestBed.inject(DOMHelperService);
        fixture = TestBed.createComponent(CustomOverlayCustomConfigTestComponent);
        component = fixture.componentInstance;
        fixture.autoDetectChanges();
    ...
    it('should ignore the defined styles and have the default ones', fakeAsync(() => {
        openOverlay();
    ...
    it('should have the defined styles for "arrowSize" and "zIndex"', fakeAsync(() => {
        openOverlay();

        expect(component.overlay.arrowSize).toEqual(10);
        expect(DOMHelper.getStyle(getOverlay().nativeElement, 'zIndex')).toEqual('100');
    }));

    it('should have the 3 defined classes', fakeAsync(() => {
        openOverlay();
        expect(component.overlay.nativeElement.classList.contains('custom-config-overlay')).toBe(true);
        expect(component.overlay.nativeElement.classList.contains('my-custom-overlay')).toBe(true);
        expect(component.overlay.nativeElement.classList.contains('custom-test-overlay')).toBe(true);
    }));
    ...
    it(`should reposition correctly on scroll down`, fakeAsync(() => {
        component.config = { ...component.config, position: 'top' };
        openOverlay();
